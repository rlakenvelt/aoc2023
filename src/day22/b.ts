...
const puzzle = 'Day 22B: Sand Slabs'
...
bricks.forEach(brick=> {
    ...
    }))  unsaveBricks.push(brick)
})

let fallingBricks: Set<Brick> = new Set()
let answer = 0;
unsaveBricks.forEach(b=>{
    fallingBricks = new Set()
    checkSupportedBricks(b)
    answer+=fallingBricks.size
})
...
function checkSupportedBricks(brick: Brick) {
    const supportedBricks = bricks.filter(b=>brick.supports(b))
    supportedBricks.forEach(s=> {
        const otherSupportingBricks = bricks.filter(b=>b!==brick&&b.supports(s)&&!fallingBricks.has(b))
        if (otherSupportingBricks.length===0) {
            fallingBricks.add(s)
            checkSupportedBricks(s)
        }
    })
}

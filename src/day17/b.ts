...
const puzzle = 'Day 17B: Clumsy Crucible'
...
function dijkstra(start: CityBlock, finish: CityBlock) {
    ...
    while (queue.length > 0) {
        ...
        if (cityBlock.x===finish.x&&cityBlock.y===finish.y) {
            if (cityBlock.constraint.length>=4)
                return cityBlock.totalEnergyLoss; 
        }
        directions.filter(d=>directionAllowed(d.symbol, cityBlock))
...
function directionAllowed(direction:string, cityBlock: CityBlock) {
    if (cityBlock.path==='') return true
    const last = cityBlock.path.slice(-1)
    ...
    const match = cityBlock.path.match(/(.)\1*$/)
    const m = match ? match[0] : ''
    if (m.length===10&&direction===last) return false
    if (m.length<4&&direction!==last) return false
    return true
}
